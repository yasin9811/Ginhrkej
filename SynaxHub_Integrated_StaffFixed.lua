...
function AutoTrash.Start()
    ...
    AutoTrash.NextAction = 0
    AutoTrash.LastTarget = nil
    AutoTrash.LastAction = 0

    AutoTrash.RunningThread = task.spawn(function()
        while AutoTrash.IsActive do
            local now = os.clock()

            if now < (AutoTrash.NextAction or 0) then
                task.wait(math.min(0.5, AutoTrash.NextAction - now))
                continue
            end
            ...
            if not char or not backpack or not rootPart or not humanoid or humanoid.Health <= 0 then
                task.wait(1)
                continue
            end
            ...
            -- Main dumpster used by SynaxHub.
            pcall(function()
                dumpster = Workspace.Map.Cells.Basement
                    ["Recyclement Room"].Props
                    ["Opened Trash"].Trash
            end)

            if dumpster then
                prompt = dumpster:FindFirstChild("Prompt")
                local interact = prompt and prompt:FindFirstChild("Interact")
                event = interact and interact:FindFirstChild("Event")

                -- Do not walk/teleport to a dumpster that is visibly cooling down.
                if prompt then
                    local enabled = prompt:GetAttribute("Enabled")
                    local onCooldown = prompt:GetAttribute("OnCooldown")

                    if enabled == false or onCooldown == true then
                        dumpster = nil
                        prompt = nil
                        event = nil
                    end
            ...
            -- Fallback to an enabled trash bin, same as the original farm.
            if not dumpster then
                refreshFarmFolders()

                if TrashesFolder then
                    for _, bin in ipairs(TrashesFolder:GetChildren()) do
                        local binPrompt = bin:FindFirstChild("Prompt")
                        if binPrompt and binPrompt:GetAttribute("Enabled") == true
                            and binPrompt:GetAttribute("OnCooldown") ~= true then

                            local interact = binPrompt:FindFirstChild("Interact")
                            local binEvent = interact and interact:FindFirstChild("Event")
            ...
            if not dumpster or not event then
                AutoTrash.NextAction = os.clock() + 0.75
                task.wait(0.25)
                continue
            end
            ...
            pcall(function()
                if dumpster:IsA("BasePart") then
                    targetCFrame = dumpster.CFrame
                    targetPosition = dumpster.Position
                else
                    targetCFrame = dumpster:GetPivot()
                    targetPosition = targetCFrame.Position
                end
            end)

            if not targetCFrame or not targetPosition then
                AutoTrash.NextAction = os.clock() + 0.75
                task.wait(0.25)
                continue
            end

            -- Never repeatedly re-teleport/re-fire the same dumpster while
            -- the previous server request is still inside its cooldown.
            if AutoTrash.LastTarget == dumpster
                and (os.clock() - AutoTrash.LastAction) < 1.5 then

                AutoTrash.NextAction = AutoTrash.LastAction + 1.5
                task.wait(0.25)
                continue
            end

            if (rootPart.Position - targetPosition).Magnitude > 7 then
                rootPart.AssemblyLinearVelocity = Vector3.zero
            ...
            -- One request per cycle. The server decides whether the throw is
            -- accepted; the client does not try to bypass that cooldown.
            local fired = false
            pcall(function()
                ...
                elseif event:IsA("BindableEvent") then
                    event:Fire()
                    fired = true
                elseif event:IsA("ProximityPrompt") and fireproximityprompt then
                    fireproximityprompt(event)
                    fired = true
                end
            end)

            if fired then
                AutoTrash.LastTarget = dumpster
                AutoTrash.LastAction = os.clock()

                -- Server-side trash cooldown guard.
                AutoTrash.NextAction = os.clock() + 1.5
                task.wait(1.5)
            else
                AutoTrash.NextAction = os.clock() + 0.75
                task.wait(0.5)
...
local function unlockFists()
    pcall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        local quests = remotes and remotes:FindFirstChild("Quests")
        local pushups = quests and quests:FindFirstChild("Pushups")
        local fn = pushups and pushups:FindFirstChild("Function")

        if fn and fn:IsA("RemoteFunction") then
            fn:InvokeServer("Submit", 300)
        end
    end)
end
...
local function getCurrentMoney()
    local attr = LocalPlayer:GetAttribute("Money")
...
local function getActiveFarmNames()
    local active = {}
    if FarmState.AutoCook then table.insert(active, "Cook") end
    if FarmState.AutoFish then table.insert(active, "Fish") end
    if AutoMine.IsActive then table.insert(active, "Mine") end
    if AutoTrash.IsActive then table.insert(active, "Trash") end
    if JanitorState.AutoFarm then table.insert(active, "Janitor") end
    return active
end

local function getActiveFarmText()
    local active = getActiveFarmNames()
    if #active == 0 then return "None" end
    return table.concat(active, " + ")
end

local function resetFarmIncome()
    local money = getCurrentMoney()
    FarmIncomeState.StartedAt = os.clock()
    FarmIncomeState.StartMoney = money
    FarmIncomeState.LastMoney = money
    FarmIncomeState.Net = 0
end

local function updateFarmIncome()
    local active = #getActiveFarmNames() > 0
    local money = getCurrentMoney()

    if active and not FarmIncomeState.Running then
        FarmIncomeState.Running = true
        resetFarmIncome()
    elseif not active then
        FarmIncomeState.Running = false
    end

    if FarmIncomeState.Running and money ~= nil then
        if FarmIncomeState.LastMoney ~= nil then
            FarmIncomeState.Net += money - FarmIncomeState.LastMoney
        end
        FarmIncomeState.LastMoney = money
    end
end
...
local function ExecuteScript()
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        ...
        anim.Completed:Connect(function()
            for _, data in pairs(ActiveDrawings) do
                ...
                if data.Bones then
                    for _, line in pairs(data.Bones) do removeDrawing(line) end
                end
            end
            ...
            pcall(function()
                local playerGui = LocalPlayer:FindFirstChildOfClass("PlayerGui")
                local editor = playerGui and playerGui:FindFirstChild("CompactHUDEditor")
    ...
    local IncomePage    = CreateTab("Farm Income", 2)
    ...
    local DashTitle = Instance.new("TextLabel", DashboardPage)
    DashTitle.Size = UDim2.new(1, -10, 0, 20)
    DashTitle.BackgroundTransparency = 1
    DashTitle.Text = "Synax Hub"
    DashTitle.TextColor3 = Config.HighlightText
    DashTitle.Font = Enum.Font.GothamBold
    DashTitle.TextSize = 15
    DashTitle.TextXAlignment = Enum.TextXAlignment.Left

    local DashSub = Instance.new("TextLabel", DashboardPage)
    DashSub.Size = UDim2.new(1, -10, 0, 30)
    DashSub.BackgroundTransparency = 1
    DashSub.Text = "Quick overview of your session, active farms and current income."
    DashSub.TextColor3 = Config.TextSecondary
    DashSub.Font = Enum.Font.Gotham
    DashSub.TextSize = 10
    DashSub.TextWrapped = true
    DashSub.TextXAlignment = Enum.TextXAlignment.Left

    local FpsLabel    = CreateCard(DashboardPage, "FPS", "calculating...")
    local PingLabel   = CreateCard(DashboardPage, "Ping", "calculating...")
    local PlayerLabel = CreateCard(DashboardPage, "Players", "0 / 0")
    local ActiveFarmLabel = CreateCard(DashboardPage, "Active Farm", "None")
    local MoneyLabel  = CreateCard(DashboardPage, "Money", "$0")
    local SessionLabel = CreateCard(DashboardPage, "Session", "0s")
    ...
    task.spawn(function()
        while MainGui.Parent do
            ...
            PlayerLabel.Text = tostring(#Players:GetPlayers()) .. " / " .. tostring(Players.MaxPlayers)

            local money = getCurrentMoney()
            MoneyLabel.Text = money and ("$" .. tostring(math.floor(money))) or "N/A"
            ActiveFarmLabel.Text = getActiveFarmText()
            SessionLabel.Text = tostring(math.floor(os.clock() - dashStart)) .. "s"
        end
    end)

    local ResetBtn = CreateButton(DashboardPage, "RESET SCRIPT", function()
        ExecuteScript()
    end)
    ...
    -- [ FARM INCOME ]
    -----------------------------------------------------------------------
    local IncomeTitle = Instance.new("TextLabel", IncomePage)
    IncomeTitle.Size = UDim2.new(1, -10, 0, 20)
    IncomeTitle.BackgroundTransparency = 1
    IncomeTitle.Text = "Farm Income"
    IncomeTitle.TextColor3 = Config.HighlightText
    IncomeTitle.Font = Enum.Font.GothamBold
    IncomeTitle.TextSize = 15
    IncomeTitle.TextXAlignment = Enum.TextXAlignment.Left

    local IncomeDesc = Instance.new("TextLabel", IncomePage)
    IncomeDesc.Size = UDim2.new(1, -10, 0, 32)
    IncomeDesc.BackgroundTransparency = 1
    IncomeDesc.Text = "Tracks net money earned while a farm is active. Spending is included in the net result."
    IncomeDesc.TextColor3 = Config.TextSecondary
    IncomeDesc.Font = Enum.Font.Gotham
    IncomeDesc.TextSize = 10
    IncomeDesc.TextWrapped = true
    IncomeDesc.TextXAlignment = Enum.TextXAlignment.Left

    local ActiveIncomeLabel = CreateCard(IncomePage, "Active Farm", "None")
    local HourlyIncomeLabel = CreateCard(IncomePage, "Hourly Income", "$0 / h")
    local NetIncomeLabel = CreateCard(IncomePage, "Net Income", "$0")
    local IncomeRuntimeLabel = CreateCard(IncomePage, "Runtime", "0s")

    local ResetIncomeBtn = CreateButton(IncomePage, "RESET INCOME", function()
        resetFarmIncome()
    end)
    ...
    task.spawn(function()
        while MainGui.Parent do
            ...
            local active = getActiveFarmText()
            local runtime = 0
            local hourly = 0
            local net = FarmIncomeState.Net or 0

            if FarmIncomeState.Running and FarmIncomeState.StartedAt > 0 then
                runtime = math.max(0, os.clock() - FarmIncomeState.StartedAt)
                if runtime > 1 then
                    hourly = net / (runtime / 3600)
                end
            end

            ActiveIncomeLabel.Text = active
            HourlyIncomeLabel.Text = "$" .. tostring(math.floor(hourly)) .. " / h"
            NetIncomeLabel.Text = (net >= 0 and "+" or "") .. "$" .. tostring(math.floor(net))
            IncomeRuntimeLabel.Text = tostring(math.floor(runtime)) .. "s"
        end
    end)

    -----------------------------------------------------------------------
    -- [ FARM TAB ]
    -- Farm features are grouped by task to keep the main Farm page clean.
    -----------------------------------------------------------------------
    local function CreateSection(parent, text)
        ...
        label.BackgroundTransparency = 1
        label.Text = text
        label.TextColor3 = Config.HighlightText
    ...
    CreateFarmSlider(FarmPage, "Fish sell interval", 5, 120, FarmState.SellInterval, 1, "s", function(value)
        FarmState.SellInterval = value
    end)

    -- FOOD / DRINK
    CreateSection(FarmPage, "🍔 AUTO EAT / DRINK")
    local HungerStatus = CreateCard(FarmPage, "Status", "Idle")
    ...
    task.spawn(function()
        while MainGui.Parent do
            ...
            if hunger ~= nil or thirst ~= nil then
                status = status .. " | H:" .. tostring(hunger or "?") .. " T:" .. tostring(thirst or "?")
            end
            HungerStatus.Text = status
        end
    end)

    CreateToggle(FarmPage, "Enable Auto Eat / Drink", false, function(state)
        AutoHungerState.Enabled = state
        if not state then AutoEatStatusLabel = "Idle" end
    end)

    CreateFarmSlider(FarmPage, "Eat when Hunger below", 5, 95, AutoHungerState.EatBelow, 1, "%", function(value)
        AutoHungerState.EatBelow = value
    end)

    CreateFarmSlider(FarmPage, "Drink when Thirst below", 5, 95, AutoHungerState.DrinkBelow, 1, "%", function(value)
        AutoHungerState.DrinkBelow = value
    end)

    -- MINING
    CreateSection(FarmPage, "⛏ AUTO MINE")
    ...
    -- TRASH
    CreateSection(FarmPage, "🗑 AUTO TRASH")
    CreateToggle(FarmPage, "Auto Trash (EXP)", false, function(state)
        if state then AutoTrash.Start() else AutoTrash.Stop() end
    end)

    -- JANITOR
    CreateSection(FarmPage, "🧹 JANITOR FARM")
    local JanitorStatus = CreateCard(FarmPage, "Status", "Idle")
    ...
    CreateToggle(FarmPage, "Janitor Farm", false, function(state)
        ...
        if state then task.spawn(janitorFarmLoop) end
    end)
    ...
    CreateFarmSlider(FarmPage, "Hits per puddle", 1, 8, JanitorState.HitsPerPuddle, 1, " fires", function(value)
        JanitorState.HitsPerPuddle = value
    end)
    CreateFarmSlider(FarmPage, "Delay between fires", 0, 2, JanitorState.CleanDelay, 0.05, "s", function(value)
        JanitorState.CleanDelay = value
    end)
    CreateFarmSlider(FarmPage, "Teleport settle time", 0, 1, JanitorState.TpWait, 0.05, "s", function(value)
    ...
    CreateToggle(EspPage, "Glow ESP", false, function(state)
    ...
    CreateSection(EspPage, "STAFF ESP")
    CreateToggle(EspPage, "Staff ESP", false, function(state)
        HubSettings.StaffEsp = state
    end)

    -----------------------------------------------------------------------
    -- [ SETTINGS / MOBILE HUD EDITOR ]
    -----------------------------------------------------------------------
    local ExtraTitle = Instance.new("TextLabel", SettingsPage)
    ...
    EditScreenGui.IgnoreGuiInset = true
    ...
    CreateToggle(SettingsPage, "Enable Mobile HUD Editor Overlay", false, function(value)
    ...
    -- Hard safety lock: the editor is strictly opt-in.  Role/team respawns,
    -- CoreGui refreshes or newly-created game HUD elements must never turn it on.
    task.spawn(function()
        while MainGui.Parent do
            task.wait(0.25)
    ...
    local PlayerGuiGuard = PlayerGui.ChildAdded:Connect(function()
        if not HUDEditorActive then
            EditScreenGui.Enabled = false
        end
    end)
    -- Team/role changes rebuild the mobile HUD in many PrisonRP versions.
    -- Close the editor during that transition instead of allowing an old
    -- overlay to pop back onto the screen.
    local function closeHUDEditor()
    ...
    local CharacterGuard = LocalPlayer.CharacterAdded:Connect(closeHUDEditor)
    local TeamGuard = LocalPlayer:GetPropertyChangedSignal("Team"):Connect(closeHUDEditor)

    local HUDInfo = Instance.new("TextLabel", SettingsPage)
    HUDInfo.Size = UDim2.new(1, -10, 0, 50)
    HUDInfo.BackgroundTransparency = 1
    HUDInfo.Text = "Manual mobile HUD editor. It stays hidden until you enable it; role changes and respawns cannot open it automatically."
    ...
    -- ESP HELPERS
    -----------------------------------------------------------------------
    -- PRISON RP / Nyxkay Studio Staff detection
    -- Uses the actual group roles instead of guessing from player names.
    local STAFF_GROUP_ID = 304256484

    local STAFF_ROLES = {
        ["Tester"] = true,
        ["Modérateur Test"] = true,
        ["Modérateur Junior"] = true,
        ["Modérateur"] = true,
        ["Modérateur Sénior"] = true,
        ["Administrateur Junior"] = true,
        ["Administrateur"] = true,
        ["Administrateur Sénior"] = true,
        ["Administrateur d'État"] = true,
    ...
    local function normalizeRole(role)
        ...
        -- Handle common Unicode apostrophe variants.
        role = role:gsub("’", "'"):gsub("`", "'")
    ...
    local function isStaffPlayer(player)
        ...
        local cached = StaffCache[player]
        if cached and (os.clock() - cached.Time) < 3 then
            return cached.Value
        end

        local result = false

        -- Primary source: Nyxkay Studio group role.
        pcall(function()
            local role = normalizeRole(player:GetRoleInGroup(STAFF_GROUP_ID))
        ...
        -- Fallbacks for the game's own attributes/teams, if present.
        if not result then
            pcall(function()
                ...
                for _, attributeName in ipairs(roleAttributes) do
                    ...
                    if type(value) == "string" then
                        ...
                        if STAFF_ROLES[normalized] then
                            result = true
                            break
                        end
                    end
        ...
        if not result then
            pcall(function()
                ...
                if team then
                    local teamName = normalizeRole(team.Name)
                    if STAFF_ROLES[teamName] then
                        result = true
                    end
                end
            end)
    ...
    local function getOrCreateNameTag(char)
        local tag = char:FindFirstChild("SynaxHubNameTag")
        if tag and tag:IsA("BillboardGui") then
            return tag
        end

        tag = Instance.new("BillboardGui")
        tag.Name = "SynaxHubNameTag"
        tag.Size = UDim2.new(0, 190, 0, 28)
    ...
    local function removeEspObjects(char)
        for _, name in ipairs({"SynaxHubNameTag", "SynaxHubHealthUI", "SynaxHubTargetLine", "SynaxHubStaffGlow"}) do
    ...
    pcall(function()
        DrawingAPI = Drawing
        DrawingSupported = DrawingAPI ~= nil and type(DrawingAPI.new) == "function"
    end)

    local function createDrawingData()
        ...
        local ok, data = pcall(function()
            ...
            for _ = 1, 6 do
                table.insert(result.Bones, DrawingAPI.new("Line"))
            end
            return result
        end)
    ...
    RunService.RenderStepped:Connect(function()
        ...
        for p, data in pairs(ActiveDrawings) do
            if not p.Parent or not Players:FindFirstChild(p.Name) then
                removeDrawing(data and data.Box)
                if data and data.Bones then
                    for _, bone in ipairs(data.Bones) do removeDrawing(bone) end
                end
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- Drawing API is optional.  Delta and many PC executors expose
                -- it, but the hub must keep running if it is unavailable.
                if not ActiveDrawings[p] and DrawingSupported then
                    ActiveDrawings[p] = createDrawingData()
                end

                local drawData = ActiveDrawings[p]
                
                -- Box ESP
                if HubSettings.BoxEsp and drawData and drawData.Box then
                    local head = char:FindFirstChild("Head")
                    if head then
                        local headPos, onScreen = currentCamera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.5, 0))
                        local rootPos, rootOnScreen = currentCamera:WorldToViewportPoint(root.Position - Vector3.new(0, 3, 0))
                    ...
                    if drawData and drawData.Box then drawData.Box.Visible = false end
                end

                -- Skeleton ESP
                if HubSettings.SkeletonEsp and drawData and humanoid and humanoid.Health > 0 then
                    local torso = char:FindFirstChild("Torso") or char:FindFirstChild("UpperTorso")
                    local head = char:FindFirstChild("Head")
                    if head and torso then
                        local hPos, hVis = currentCamera:WorldToViewportPoint(head.Position)
                        local tPos, tVis = currentCamera:WorldToViewportPoint(torso.Position)
                    ...
                    if drawData and drawData.Bones then
                ...
                -- Name + distance / staff marker
                local staffDetected = HubSettings.StaffEsp and isStaffPlayer(p)
                local nameTag = char:FindFirstChild("SynaxHubNameTag")
                if HubSettings.NameDistanceEsp or staffDetected then
                    nameTag = getOrCreateNameTag(char)
                    local label = nameTag:FindFirstChild("Text")
                    if label then
                        local distance = math.floor((root.Position - currentCamera.CFrame.Position).Magnitude)
                        if staffDetected then
                            local staffRole = ""
                            pcall(function()
                                staffRole = normalizeRole(p:GetRoleInGroup(STAFF_GROUP_ID))
                            end)

                            if staffRole == "" or not STAFF_ROLES[staffRole] then
                                staffRole = "STAFF"
                            end

                            label.Text = "STAFF • " .. p.DisplayName .. "  •  " .. distance .. "m"
                            label.TextColor3 = Color3.fromRGB(255, 90, 90)
                        else
                            label.Text = p.DisplayName .. "  •  " .. distance .. "m"
                            label.TextColor3 = Color3.fromRGB(255, 255, 255)
                        end
                    end
                elseif nameTag then
                ...
                -- Staff highlight
                local staffGlow = char:FindFirstChild("SynaxHubStaffGlow")
                if staffDetected then
                    if not staffGlow then
                        staffGlow = Instance.new("Highlight")
                        staffGlow.Name = "SynaxHubStaffGlow"
                        staffGlow.FillColor = Color3.fromRGB(255, 60, 60)
                        staffGlow.OutlineColor = Color3.fromRGB(255, 255, 255)
                        staffGlow.FillTransparency = 0.55
                        staffGlow.OutlineTransparency = 0
                        staffGlow.Parent = char
                    end
                elseif staffGlow then
                    staffGlow:Destroy()
                end
    ...
    Players.PlayerAdded:Connect(function(player)
        StaffCache[player] = nil
    end)

    Players.PlayerRemoving:Connect(function(player)
        local data = ActiveDrawings[player]
        if data then
            removeDrawing(data.Box)
            if data.Bones then
                for _, bone in ipairs(data.Bones) do removeDrawing(bone) end
