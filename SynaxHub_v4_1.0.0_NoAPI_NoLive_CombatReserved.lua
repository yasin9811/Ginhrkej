...
local function readPlayerStat(statName)
    local attr = LocalPlayer:GetAttribute(statName)
    if type(attr) == "number" then return attr end
    if type(attr) == "string" then
        local parsed = tonumber(attr)
    ...
    for _, container in ipairs(containers) do
        if container then
            local obj = container:FindFirstChild(statName)
...
local function findAutoEatTool(names)
    local function matches(tool)
        ...
        if names[tool.Name] then return true end
        local n = string.lower(tool.Name)
        if names == FOOD_TOOLS then
            return n:find("food") or n:find("eat") or n:find("cereal") or n:find("popcorn") or n:find("plate")
        end
        return n:find("drink") or n:find("water") or n:find("soda") or n:find("cola") or n:find("juice")
    end
...
local function consumeAutoHunger(kind, toolNames, statName, thresholdOf)
    ...
    while RunningAutoHunger and AutoHungerState.Enabled and actions < 50 and misses < 3 do
        ...
        local stat = readPlayerStat(statName)
        ...
        if not tool then
            ...
            local money = readPlayerStat("Money") or readPlayerStat("Cash") or readPlayerStat("Coins") or 0
            if money < nearest.price then
        ...
        local before = readPlayerStat(statName) or 0
        ...
        while RunningAutoHunger and AutoHungerState.Enabled and os.clock() - t1 < 4 do
            ...
            if (readPlayerStat(statName) or 0) ~= before then break end
            task.wait(0.1)
        end

        if (readPlayerStat(statName) or 0) ~= before then
            misses = 0
        else
            misses += 1
        end
...
local function StartAutoHungerLoop()
    ...
    AutoHungerThread = task.spawn(function()
        while RunningAutoHunger and AutoHungerState.Enabled do
            ...
            pcall(function()
                local hunger = readPlayerStat("Hunger")
                local thirst = readPlayerStat("Thirst")
...
local JanitorState = {
    ...
    local function FarmEarning_ReadMoney()
        local attrNames = {"Cash", "Money", "Coins", "Balance", "Currency", "Credits"}
...
local function ExecuteScript()
    ...
    NotifFrame.Size = UDim2.new(0, 365, 0, 105)
    NotifFrame.Position = UDim2.new(1, 385, 1, -115)
    ...
    NotifTitle.Text = "C11 SYNAX • DEVELOPER"
    ...
    NotifDesc.Size = UDim2.new(1, -25, 0, 38)
    NotifDesc.Position = UDim2.new(0, 20, 0, 29)
    NotifDesc.BackgroundTransparency = 1
    NotifDesc.Text = "Developer: Devilmain3131\nFollow the developer's account to support C11 Synax."
    ...
    local DeveloperProfile = Instance.new("TextButton", NotifFrame)
    DeveloperProfile.Size = UDim2.new(1, -25, 0, 24)
    DeveloperProfile.Position = UDim2.new(0, 20, 1, -30)
    DeveloperProfile.BackgroundColor3 = Config.ContainerColor
    DeveloperProfile.BackgroundTransparency = 0.1
    DeveloperProfile.Text = "        ↗  Follow Developer"
    DeveloperProfile.TextColor3 = Config.TextPrimary
    DeveloperProfile.Font = Enum.Font.GothamBold
    DeveloperProfile.TextSize = 9
    DeveloperProfile.TextXAlignment = Enum.TextXAlignment.Left
    DeveloperProfile.AutoButtonColor = false
    Instance.new("UICorner", DeveloperProfile).CornerRadius = UDim.new(0, 5)

    local DeveloperAvatar = Instance.new("ImageLabel", DeveloperProfile)
    DeveloperAvatar.Size = UDim2.fromOffset(20, 20)
    DeveloperAvatar.Position = UDim2.new(0, 3, 0.5, -10)
    DeveloperAvatar.BackgroundTransparency = 1
    DeveloperAvatar.Image = "rbxthumb://type=AvatarHeadShot&id=11298109458&w=100&h=100"
    Instance.new("UICorner", DeveloperAvatar).CornerRadius = UDim.new(1, 0)
    DeveloperProfile.MouseButton1Click:Connect(function()
        local url = "https://www.roblox.com/tr/users/11298109458/profile"
        pcall(function()
            if setclipboard then setclipboard(url) end
        end)
        pcall(function()
            if toclipboard then toclipboard(url) end
        end)
        pcall(function()
            StarterGui:SetCore("SendNotification", {Title="C11 SYNAX", Text="Developer profile link copied.", Duration=3})
        end)
    end)

    TweenService:Create(NotifFrame, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Position = UDim2.new(1, -380, 1, -115)
    }):Play()
    ...
    task.delay(15, function()
        if NotifFrame and NotifFrame.Parent then
            local outTween = TweenService:Create(NotifFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Position = UDim2.new(1, 385, 1, -115)
            })
            outTween:Play()
    ...
    -----------------------------------------------------------------------
    local CombatNotice = Instance.new("Frame", CombatPage)
    CombatNotice.Size = UDim2.new(1, -10, 0, 58)
    CombatNotice.BackgroundColor3 = Config.ContainerColor
    CombatNotice.BackgroundTransparency = 0.2
    Instance.new("UICorner", CombatNotice).CornerRadius = UDim.new(0, 6)

    local CombatNoticeTitle = Instance.new("TextLabel", CombatNotice)
    CombatNoticeTitle.Size = UDim2.new(1, -20, 0, 22)
    CombatNoticeTitle.Position = UDim2.new(0, 10, 0, 5)
    CombatNoticeTitle.BackgroundTransparency = 1
    CombatNoticeTitle.Text = "Combat"
    CombatNoticeTitle.TextColor3 = Config.HighlightText
    CombatNoticeTitle.Font = Enum.Font.GothamBold
    CombatNoticeTitle.TextSize = 12
    CombatNoticeTitle.TextXAlignment = Enum.TextXAlignment.Left

    local CombatNoticeDesc = Instance.new("TextLabel", CombatNotice)
    CombatNoticeDesc.Size = UDim2.new(1, -20, 0, 24)
    CombatNoticeDesc.Position = UDim2.new(0, 10, 0, 28)
    CombatNoticeDesc.BackgroundTransparency = 1
    CombatNoticeDesc.Text = "Combat systems are currently unavailable in v1.0.0."
    CombatNoticeDesc.TextColor3 = Config.TextSecondary
    CombatNoticeDesc.Font = Enum.Font.Gotham
    CombatNoticeDesc.TextSize = 10
    CombatNoticeDesc.TextWrapped = true
    CombatNoticeDesc.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local StaffCharacterConnections = {}

    local function watchStaffCharacter(player)
        if not player or player == LocalPlayer then return end

        if StaffCharacterConnections[player] then
            StaffCharacterConnections[player]:Disconnect()
            StaffCharacterConnections[player] = nil
        end

        StaffCharacterConnections[player] = player.CharacterAdded:Connect(function()
    ...
    -- [ FARM EARNING TAB ]
    -----------------------------------------------------------------------
    local FarmEarningMoneyLabel = CreateCard(FarmEarningPage, "Current Money", tostring(math.floor(FarmEarning_ReadMoney() or 0)))
...
local FarmTitle = Instance.new("TextLabel", FarmPage)
    ...
    -- feature logic, access checks, and staff system remain untouched.
    -----------------------------------------------------------------------
    local AllPages = {
        DashboardPage,
        InfoPage,
