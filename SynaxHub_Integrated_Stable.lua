...
local function ExecuteScript()
    ...
    local function isStaffPlayer(player)
        ...
        if cached and os.clock() - cached.Time < 2 then
            return cached.Value
        end

        local staffWords = {
            "staff", "admin", "administrator", "moderator", "mod",
            "developer", "dev", "owner", "manager", "support"
        }

        local function hasStaffWord(value)
            if value == nil then return false end
            local text = string.lower(tostring(value))
            for _, word in ipairs(staffWords) do
        ...
        if hasStaffWord(player.Team and player.Team.Name) then
            result = true
        end

        if not result then
            for _, attrName in ipairs({"Staff", "IsStaff", "IsAdmin", "IsModerator", "StaffRole", "Role", "Team"}) do
                local value = player:GetAttribute(attrName)
                if value == true or hasStaffWord(value) then
                    result = true
                    break
                end
            end
        ...
        if not result then
            pcall(function()
                if game.CreatorType == Enum.CreatorType.Group and game.CreatorId and game.CreatorId > 0 then
                    local role = player:GetRoleInGroup(game.CreatorId)
                    if hasStaffWord(role) then
                        result = true
                    end
                end
            end)
