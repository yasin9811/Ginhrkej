    ...
    while true do
        ...
        pcall(function()
            ...
            if rod.Parent ~= character then
                -- Normal equip first, then a safe fallback if the game's equip
                -- handler fails to move the rod out of Backpack.
                for _ = 1, 3 do
...
local function ExecuteScript()
    ...
    local HUDStateGuard = PlayerGui.ChildAdded:Connect(function()
    ...
    CreateToggle(CombatPage, "Hold RMB to Aim", true, function(state)
    ...
    CreateToggle(CombatPage, "Sticky Target", true, function(state)
        AimbotState.Sticky = state
    end)

    CreateToggle(CombatPage, "FOV", true, function(state)
        AimbotState.FOVEnabled = state
    end)

    CreateSlider(CombatPage, "FOV Size", 40, 500, 180, function(value)
        AimbotState.FOV = value
    end)

    CreateSlider(CombatPage, "Smoothness", 1, 100, 18, function(value)
        AimbotState.Smoothness = math.clamp(value / 100, 0.01, 1)
    end)

    CreateSlider(CombatPage, "Prediction", 0, 30, 8, function(value)
        AimbotState.Prediction = value / 100
    end)

    local TargetPartLabel = Instance.new("TextLabel", CombatPage)
    TargetPartLabel.Size = UDim2.new(1, -10, 0, 28)
    TargetPartLabel.BackgroundTransparency = 1
    TargetPartLabel.Text = "Target: Head • PC / Mouse2"
    ...
    -- Throttled to reduce frame spikes when toggling ESP/utility features.
    local LastVisualUpdate = 0
    local VisualInterval = 1 / 20
    ...
    local function cleanupPlayerVisual(p)
        ...
        if p.Character then
            local glow = p.Character:FindFirstChild("SynaxHubGlow")
            if glow then glow:Destroy() end
            local healthUi = p.Character:FindFirstChild("SynaxHubHealthUI")
            if healthUi then healthUi:Destroy() end
            local linePart = p.Character:FindFirstChild("SynaxHubTargetLine")
    ...
    RunService.RenderStepped:Connect(function()
        ...
        -- Utility features only do work while enabled.
    ...
    AimbotState.Connection = RunService.RenderStepped:Connect(function()
        ...
        if AimbotState.HoldMouse2 and not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            AimbotState.Target = nil
            return
        end
