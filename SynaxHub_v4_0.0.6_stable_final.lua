...
-- ============================================================================
-- SYNAX HUB LOCALIZATION
-- ============================================================================
local LanguageState = { Current = "English" }
local L = {
    English = {
        ON="ON", OFF="OFF", Beta="Official", Settings="Settings", Information="Information",
        Dashboard="Dashboard", Farm="Farm", ["Farm Earnings"]="Farm Earnings", ["Auto Eat"]="Auto Eat",
        Combat="Combat", ESP="ESP", Staff="Staff", Extra="Extra", ["Current Language"]="Current Language",
        ["Server Players"]="Server Players", ["FPS Rate"]="FPS Rate", ["MS (Ping)"]="MS (Ping)",
        ["Active Friends"]="Active Friends", ["Auto Cook"]="Auto Cook", ["Fish Farm"]="Fish Farm",
        ["Auto Sell Fish"]="Auto Sell Fish", ["Auto Mine"]="Auto Mine", ["Auto Trash (EXP)"]="Auto Trash (EXP)",
        ["Special Player"]="Special Player", ["No player selected"]="No player selected"
    },
    Turkish = {}, French = {}, German = {}, Spanish = {}, Portuguese = {}, Russian = {}, Arabic = {}, Polish = {}, Dutch = {},
    Azerbaijani = {}, Romanian = {}, Albanian = {}, Italian = {}, Norwegian = {}
}
for _, name in ipairs({"Turkish","French","German","Spanish","Portuguese","Russian","Arabic","Polish","Dutch","Azerbaijani","Romanian","Albanian","Italian","Norwegian"}) do
    for k,v in pairs(L.English) do L[name][k] = v end
end

local SupportedLanguages = {
    {Name="English", Flag="🇬🇧"}, {Name="Turkish", Flag="🇹🇷"}, {Name="French", Flag="🇫🇷"},
    {Name="German", Flag="🇩🇪"}, {Name="Spanish", Flag="🇪🇸"}, {Name="Portuguese", Flag="🇵🇹"},
    {Name="Russian", Flag="🇷🇺"}, {Name="Arabic", Flag="🇸🇦"}, {Name="Polish", Flag="🇵🇱"},
    {Name="Dutch", Flag="🇳🇱"}, {Name="Azerbaijani", Flag="🇦🇿"}, {Name="Romanian", Flag="🇷🇴"},
    {Name="Albanian", Flag="🇦🇱"}, {Name="Italian", Flag="🇮🇹"}, {Name="Norwegian", Flag="🇳🇴"}
}
...
local Config = {
    ...
    Version = "v1.0.0",
    ReleaseName = "SynaxHub Global Update • v1.0.0 (Official Release)",
    DeveloperUserId = 11298109458
}
...
-- Central connection registry: every long-lived RenderStepped/Heartbeat/Stepped
-- connection is tracked so UI shutdown can disconnect everything cleanly.
...
local function disconnectRuntimeConnections()
    for i = #RuntimeConnections, 1, -1 do
        local connection = RuntimeConnections[i]
...
local function safeChangeState(humanoid)
...
local function safeTeleportRoot(rootPart, cframe)
    if not rootPart or not rootPart.Parent or not cframe then return false end
    local character = rootPart.Parent
    local humanoid = character and character:FindFirstChildOfClass("Humanoid")
    safeChangeState(humanoid)
    pcall(function()
...
local function findToolEvent()
    ...
    -- Small, bounded fallback: only inspect known remote containers.
    local candidates = {
        remotes,
        remotes and remotes:FindFirstChild("Tool"),
        ReplicatedStorage:FindFirstChild("Tool"),
        ReplicatedStorage:FindFirstChild("RemoteEvents")
    }

    for _, container in ipairs(candidates) do
        if container then
            local direct = container:FindFirstChild("Event")
            if direct and direct:IsA("RemoteEvent") then
                return direct
            end
            local toolEvent = container:FindFirstChild("ToolEvent")
...
local RunningAutoHunger = false
local AutoHungerThread = nil
        ...
        for _, inst in ipairs(container:GetChildren()) do
            if inst:IsA("RemoteEvent") and inst.Parent and inst.Parent:IsA("Configuration") then
                ...
                if text:find("Buy Food") or text:find("Buy Drink") then
                    ...
                    if machine and not seen[inst] then
                        seen[inst] = true
                        table.insert(list, {
                            event = inst,
            ...
            elseif inst:IsA("Folder") or inst:IsA("Model") then
                ...
                if lower:find("vending") or lower:find("food") or lower:find("drink") then
                    inspectContainer(inst)
                end
            end
        end
    end

    -- Prefer explicit vending containers. This avoids an unbounded Workspace scan.
    local candidates = {
        Workspace:FindFirstChild("VendingMachines"),
        Workspace:FindFirstChild("Vending"),
...
local function StartAutoHungerLoop()
    if AutoHungerThread then return end
    RunningAutoHunger = true
    AutoHungerThread = task.spawn(function()
        while RunningAutoHunger and AutoHungerState.Enabled do
            task.wait(1)
            if not RunningAutoHunger or not AutoHungerState.Enabled then break end
            pcall(function()
        ...
        AutoHungerThread = nil
    end)
end

local function StopAutoHungerLoop()
    RunningAutoHunger = false
    AutoHungerThread = nil
end
...
-- Live earnings are calculated from the player's cash value at script start.
local FarmEarnings = {
    StartCash = nil,
    AutoFish = 0,
    AutoCook = 0,
...
local function getCashValue()
    local attrNames = {"Money", "Cash", "cash", "money"}
    ...
    if leaderstats then
        ...
        for _, value in ipairs(leaderstats:GetChildren()) do
            if value:IsA("IntValue") or value:IsA("NumberValue") then
                local n = string.lower(value.Name)
                if n:find("cash") or n:find("money") then return value.Value end
            end
        end
    end
    return nil
...
local function formatMoney(value)
    ...
    while true do
        local new, count = str:gsub("^(%d+)(%d%d%d)", "%1,%2")
        str = new
...
local function mopPuddle(puddle)
    ...
    while JanitorState.AutoFarm do
        ...
        if (hrp.Position - puddle.Position).Magnitude > 8 then
            hrp.AssemblyLinearVelocity = Vector3.zero
            safeTeleportRoot(hrp, CFrame.new(puddle.Position + Vector3.new(0, 3.2, 0)))
...
local function cleanNearestPuddle()
    ...
    safeTeleportRoot(hrp, CFrame.new(nearest.Position + Vector3.new(0, 3.5, 0)))
    task.wait(JanitorState.TpWait)
...
local function teleportToJanitor()
    ...
    if hrp then
        safeTeleportRoot(hrp, CFrame.new(91.27, 13.8, -693.9))
    end
end
    ...
    task.spawn(function()
        for _, puddle in ipairs(getPuddles()) do
            if not JanitorState.AutoFarm then
                safeTeleportRoot(hrp, CFrame.new(puddle.Position + Vector3.new(0, 3.5, 0)))
                task.wait(0.5)
            end
        end
    end)
...
function AutoMine.Start()
    ...
    -- Respawn-safe noclip state: old BasePart references are discarded and
    -- the new character is rebuilt automatically by the Stepped handler.
    if AutoMine.CharacterConnection then
        pcall(function() AutoMine.CharacterConnection:Disconnect() end)
    end
    AutoMine.CharacterConnection = trackConnection(LocalPlayer.CharacterAdded:Connect(function()
        clearFarmTable(AutoMine.OriginalCollisions)
        task.defer(function()
            if AutoMine.IsActive and LocalPlayer.Character then
    ...
    AutoMine.NoclipConnection = trackConnection(RunService.Stepped:Connect(function()
    ...
    AutoMine.RunningThread = task.spawn(function()
        while AutoMine.IsActive do
            ...
            if targetRock then
                    AutoMine.LockConnection = trackConnection(RunService.Heartbeat:Connect(function()
                    if targetRock and targetRock.Parent and rootPart and rootPart.Parent then
                        safeTeleportRoot(rootPart, targetRock.CFrame * CFrame.new(0, 3, 0))
                    end
                end))
...
function AutoMine.Stop()
    ...
    if AutoMine.CharacterConnection then
        AutoMine.CharacterConnection:Disconnect()
        AutoMine.CharacterConnection = nil
    end
                    ...
                    if not trashTool then
                        ...
                        if TrashesFolder then
                            ...
                            if activeBin then
                                ...
                                if target then
                                    rootPart.AssemblyLinearVelocity = Vector3.zero
                                    safeTeleportRoot(rootPart, target.CFrame * CFrame.new(0, 2, 0))
                        ...
                        if dumpster then
                            rootPart.AssemblyLinearVelocity = Vector3.zero
                            safeTeleportRoot(rootPart, dumpster.CFrame * CFrame.new(0, 2, 0))
...
local function StopAutoCookLoop()
...
local function StartAutoFishLoop()
    ...
    AutoFishThread = task.spawn(function()
        while FarmState.AutoFish do
            pcall(function()
                if not loadFishingModules() then task.wait(1); return end
                ...
                if rod.Parent ~= character then
                    ...
                    for _ = 1, 5 do
                        ...
                        if rod.Parent == LocalPlayer.Backpack then pcall(function() humanoid:EquipTool(rod) end) end
                        task.wait(0.2)
                    end
                ...
                if rod.Parent ~= character then task.wait(1); return end
...
local function StopAutoFishLoop()
    ...
    AutoFishThread = nil
    pcall(function() if MinigameSystem then MinigameSystem:SetHolding(false) end end)
    local char = LocalPlayer.Character
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if humanoid then
...
local function StartAutoSellLoop()
    ...
    AutoSellThread = task.spawn(function()
        while FarmState.AutoSell do
            ...
            pcall(function()
                if not FishingSystem then FishingSystem = ReplicatedStorage:FindFirstChild("FishingSystem") end
                ...
                if sellAll and sellAll:IsA("RemoteFunction") then sellAll:InvokeServer() end
            end)
        end
        AutoSellThread = nil
    end)
...
local function ExecuteScript()
    ...
    NotifStroke.Color = Config.HighlightText
    ...
    AccentLine.BackgroundColor3 = Config.HighlightText
    ...
    NotifTitle.Text = "Developer • Devilmain3131"
    NotifTitle.TextColor3 = Config.HighlightText
    NotifTitle.Font = Enum.Font.GothamBold
    NotifTitle.TextSize = 11
    NotifTitle.TextXAlignment = Enum.TextXAlignment.Left

    local DevAvatar = Instance.new("ImageLabel", NotifFrame)
    DevAvatar.Size = UDim2.fromOffset(42, 42)
    DevAvatar.Position = UDim2.new(1, -52, 0, 9)
    DevAvatar.BackgroundTransparency = 1
    DevAvatar.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(Config.DeveloperUserId) .. "&w=150&h=150"
    ...
    NotifDesc.Text = "Support the developer by following Devilmain3131!"
    ...
    local function CreateTab(name, layoutOrder)
        ...
        TabBtn.MouseButton1Click:Connect(function()
            for _, btn in pairs(Tabs) do
                TweenService:Create(btn, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Config.ContainerColor}):Play()
            end
            for _, p in pairs(Pages) do
                if p ~= Page then p.Visible = false end
            end
            TweenService:Create(TabBtn, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Config.ContainerActive}):Play()
            Page.Visible = true
            Page.Position = UDim2.new(0, 18, 0, 0)
            Page.CanvasPosition = Vector2.zero
            TweenService:Create(Page, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, 0, 0, 0)}):Play()
        end)
    ...
    local FarmEarningsPage = CreateTab("Farm Earnings", 3)
    ...
    -- [ FARM EARNINGS ]
    -----------------------------------------------------------------------
    local EarningsTitle = Instance.new("TextLabel", FarmEarningsPage)
    EarningsTitle.Size = UDim2.new(1, -10, 0, 24)
    EarningsTitle.BackgroundTransparency = 1
    EarningsTitle.Text = "Farm Earnings • LIVE"
    EarningsTitle.TextColor3 = Config.HighlightText
    EarningsTitle.Font = Enum.Font.GothamBold
    EarningsTitle.TextSize = 13
    EarningsTitle.TextXAlignment = Enum.TextXAlignment.Left

    local EarningsInfo = Instance.new("TextLabel", FarmEarningsPage)
    EarningsInfo.Size = UDim2.new(1, -10, 0, 30)
    EarningsInfo.BackgroundTransparency = 1
    EarningsInfo.Text = "Live cash delta while SynaxHub farms are active."
    EarningsInfo.TextColor3 = Config.TextSecondary
    EarningsInfo.Font = Enum.Font.Gotham
    EarningsInfo.TextSize = 9
    EarningsInfo.TextXAlignment = Enum.TextXAlignment.Left

    local FishEarningsLabel = CreateCard(FarmEarningsPage, "Auto Fish Earnings", "$0")
    local CookEarningsLabel = CreateCard(FarmEarningsPage, "Auto Cook Earnings", "$0")
    local TotalEarningsLabel = CreateCard(FarmEarningsPage, "Total Farmed Amount", "$0")
    local LiveCashLabel = CreateCard(FarmEarningsPage, "Current Cash", "$0")

    FarmEarnings.StartCash = getCashValue() or 0
    FarmEarnings.LastCash = FarmEarnings.StartCash
    ...
    task.spawn(function()
        while MainGui.Parent do
            ...
            if cash ~= nil then
                if FarmEarnings.LastCash == nil then FarmEarnings.LastCash = cash end
                local delta = cash - FarmEarnings.LastCash
                if delta > 0 then
                    if FarmState.AutoFish then FarmEarnings.AutoFish += delta end
                    if FarmState.AutoCook then FarmEarnings.AutoCook += delta end
                end
                FarmEarnings.LastCash = cash
                FarmEarnings.Total = math.max(0, cash - (FarmEarnings.StartCash or cash))
                LiveCashLabel.Text = formatMoney(cash)
                FishEarningsLabel.Text = formatMoney(FarmEarnings.AutoFish)
                CookEarningsLabel.Text = formatMoney(FarmEarnings.AutoCook)
                TotalEarningsLabel.Text = formatMoney(FarmEarnings.Total)
            end
        end
    end)
    ...
    CreateToggle(FarmPage, "Auto Cook", false, function(state)
        ...
        if state then StartAutoCookLoop() else StopAutoCookLoop() end
    end)

    CreateToggle(FarmPage, "Fish Farm", false, function(state)
        FarmState.AutoFish = state
        if state then StartAutoFishLoop() else StopAutoFishLoop() end
    end)

    CreateToggle(FarmPage, "Auto Sell Fish", false, function(state)
        FarmState.AutoSell = state
        if state then StartAutoSellLoop() else StopAutoSellLoop() end
    end)
    ...
    CreateButton(FarmPage, "Teleport to Fishing Area", function()
        ...
        if hrp and fz then
            pcall(function()
                safeTeleportRoot(hrp, fz:GetPivot() + Vector3.new(0, 5, 0))
            end)
        end
    end)
    ...
    CreateToggle(EspPage, "Glow ESP (All White)", false, function(state)
        ...
        if not state then for _, p in ipairs(Players:GetPlayers()) do cleanupPlayerVisual and cleanupPlayerVisual(p) end end
    end)
    ...
    local function teleportToBlackMarket()
        ...
        if npc and hrp then
            ...
            if npcHRP then
                ...
                hrp.AssemblyLinearVelocity = Vector3.zero
                safeTeleportRoot(hrp, CFrame.lookAt(frontPosition.Position, npcHRP.Position))
            end
        end
    end
    ...
    -- Special Player dropdown with live avatar thumbnails.
    local SpecialPlayerHolder = Instance.new("Frame", CombatPage)
    SpecialPlayerHolder.Size = UDim2.new(1, -10, 0, 38)
    SpecialPlayerHolder.BackgroundColor3 = Config.ContainerColor
    SpecialPlayerHolder.BackgroundTransparency = 0.12
    Instance.new("UICorner", SpecialPlayerHolder).CornerRadius = UDim.new(0, 7)

    local SpecialPlayerButton = Instance.new("TextButton", SpecialPlayerHolder)
    SpecialPlayerButton.Size = UDim2.new(1, 0, 1, 0)
    SpecialPlayerButton.BackgroundTransparency = 1
    SpecialPlayerButton.Text = "  " .. Translate("Special Player") .. ": " .. Translate("No player selected")
    SpecialPlayerButton.TextColor3 = Config.TextPrimary
    SpecialPlayerButton.Font = Enum.Font.GothamBold
    SpecialPlayerButton.TextSize = 10
    SpecialPlayerButton.TextXAlignment = Enum.TextXAlignment.Left
    SpecialPlayerButton.AutoButtonColor = false

    local SpecialArrow = Instance.new("TextLabel", SpecialPlayerButton)
    SpecialArrow.Size = UDim2.new(0, 28, 1, 0)
    SpecialArrow.Position = UDim2.new(1, -34, 0, 0)
    SpecialArrow.BackgroundTransparency = 1
    SpecialArrow.Text = "▼"
    SpecialArrow.TextColor3 = Config.HighlightText
    SpecialArrow.Font = Enum.Font.GothamBold
    SpecialArrow.TextSize = 11

    local SpecialDropdown = Instance.new("Frame", CombatPage)
    SpecialDropdown.Size = UDim2.new(1, -10, 0, 0)
    SpecialDropdown.BackgroundColor3 = Config.ContainerColor
    SpecialDropdown.BackgroundTransparency = 0.05
    SpecialDropdown.Visible = false
    SpecialDropdown.ClipsDescendants = true
    Instance.new("UICorner", SpecialDropdown).CornerRadius = UDim.new(0, 7)

    local SpecialScroll = Instance.new("ScrollingFrame", SpecialDropdown)
    SpecialScroll.Size = UDim2.fromScale(1, 1)
    SpecialScroll.BackgroundTransparency = 1
    SpecialScroll.BorderSizePixel = 0
    SpecialScroll.ScrollBarThickness = 3
    SpecialScroll.ScrollBarImageColor3 = Config.HighlightText
    SpecialScroll.CanvasSize = UDim2.new(0, 0, 0, 0)

    local SpecialLayout = Instance.new("UIListLayout", SpecialScroll)
    SpecialLayout.Padding = UDim.new(0, 3)
    SpecialLayout.SortOrder = Enum.SortOrder.LayoutOrder

    local SpecialOpen = false
    local SelectedSpecialPlayer = nil

    local function getThumb(userId)
        local ok, content = pcall(function()
            local image, ready = Players:GetUserThumbnailAsync(userId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)
            return image, ready
        end)
        return ok and content or "rbxasset://textures/ui/GuiImagePlaceholder.png"
    end

    local function refreshSpecialPlayers()
        for _, child in ipairs(SpecialScroll:GetChildren()) do
            if not child:IsA("UIListLayout") then child:Destroy() end
        end
        local players = Players:GetPlayers()
        table.sort(players, function(a,b) return string.lower(a.DisplayName) < string.lower(b.DisplayName) end)
        for index, player in ipairs(players) do
            if player ~= LocalPlayer then
                local row = Instance.new("TextButton", SpecialScroll)
                row.Size = UDim2.new(1, -6, 0, 38)
                row.BackgroundColor3 = Config.ContainerColor
                ...
                avatar.Image = getThumb(player.UserId)
                ...
                row.MouseButton1Click:Connect(function()
                    SelectedSpecialPlayer = player
                    SpecialPlayerButton.Text = "  " .. Translate("Special Player") .. ": " .. player.DisplayName
                    SpecialOpen = false
                    SpecialDropdown.Visible = false
                    SpecialDropdown.Size = UDim2.new(1, -10, 0, 0)
                    SpecialArrow.Text = "▼"
                    -- Sticky aim can immediately use the selected player.
                    AimbotState.Target = player
                end)
            end
        end
        SpecialScroll.CanvasSize = UDim2.new(0, 0, 0, SpecialLayout.AbsoluteContentSize.Y + 6)
    end

    SpecialPlayerButton.MouseButton1Click:Connect(function()
        SpecialOpen = not SpecialOpen
        if SpecialOpen then refreshSpecialPlayers() end
        SpecialDropdown.Visible = SpecialOpen
        SpecialDropdown.Size = UDim2.new(1, -10, 0, SpecialOpen and math.min(190, math.max(42, SpecialLayout.AbsoluteContentSize.Y + 6)) or 0)
        SpecialArrow.Text = SpecialOpen and "▲" or "▼"
    end)

    Players.PlayerAdded:Connect(function() if SpecialOpen then refreshSpecialPlayers() end end)
    Players.PlayerRemoving:Connect(function(player)
        if SelectedSpecialPlayer == player then
            SelectedSpecialPlayer = nil
            AimbotState.Target = nil
            SpecialPlayerButton.Text = "  " .. Translate("Special Player") .. ": " .. Translate("No player selected")
        end
        if SpecialOpen then refreshSpecialPlayers() end
    ...
    -----------------------------------------------------------------------
    -- [ INFORMATION TAB • OFFICIAL RELEASE ]
    -----------------------------------------------------------------------
    local function InfoCard(parent, title, value, icon, accentA, accentB, height)
        local card = Instance.new("Frame", parent)
        card.Size = UDim2.new(1, -10, 0, height or 64)
        card.BackgroundColor3 = Config.ContainerColor
        ...
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, accentA or Color3.fromRGB(35, 55, 95)),
            ColorSequenceKeypoint.new(1, accentB or Color3.fromRGB(70, 35, 100))
        })
        gradient.Transparency = NumberSequence.new({
            NumberSequenceKeypoint.new(0, 0.55), NumberSequenceKeypoint.new(1, 0.8)
        })
        local stroke = Instance.new("UIStroke", card)
        stroke.Color = accentA or Config.HighlightText
        stroke.Transparency = 0.45
        stroke.Thickness = 1
        local iconLabel = Instance.new("TextLabel", card)
        iconLabel.Size = UDim2.fromOffset(42, 42)
        iconLabel.Position = UDim2.new(0, 9, 0.5, -21)
        iconLabel.BackgroundColor3 = accentA or Config.HighlightText
        iconLabel.BackgroundTransparency = 0.25
        iconLabel.Text = icon or "◆"
        iconLabel.TextColor3 = Config.TextPrimary
        iconLabel.Font = Enum.Font.GothamBold
        iconLabel.TextSize = 18
        Instance.new("UICorner", iconLabel).CornerRadius = UDim.new(0, 9)
        local titleLabel = Instance.new("TextLabel", card)
        titleLabel.Size = UDim2.new(1, -70, 0, 19)
        titleLabel.Position = UDim2.new(0, 61, 0, 10)
        titleLabel.BackgroundTransparency = 1
        titleLabel.Text = title
        titleLabel.TextColor3 = accentA or Config.HighlightText
        titleLabel.Font = Enum.Font.GothamBold
        titleLabel.TextSize = 10
        titleLabel.TextXAlignment = Enum.TextXAlignment.Left
        local valueLabel = Instance.new("TextLabel", card)
        valueLabel.Size = UDim2.new(1, -70, 0, height and height - 34 or 24)
        valueLabel.Position = UDim2.new(0, 61, 0, 29)
        valueLabel.BackgroundTransparency = 1
        valueLabel.Text = value
        valueLabel.TextColor3 = Config.TextPrimary
    ...
    local ReleaseHeader = InfoCard(InfoPage, "OFFICIAL RELEASE", Config.ReleaseName, "✦", Color3.fromRGB(65, 145, 255), Color3.fromRGB(120, 60, 255), 78)
    local ProjectInfo = InfoCard(InfoPage, "PROJECT", "SynaxHub is built for the PrisonRP experience on Roblox. Feature behavior depends on the current game environment.", "◈", Color3.fromRGB(80, 140, 255), Color3.fromRGB(80, 80, 180), 72)
    local UpdateInfo = InfoCard(InfoPage, "GLOBAL UPDATE", "Modernized interface, live farm earnings, player selector, smoother navigation, refreshed ESP cleanup and improved legit targeting.", "↻", Color3.fromRGB(145, 80, 255), Color3.fromRGB(70, 100, 255), 76)
    local DeveloperInfo = InfoCard(InfoPage, "DEVELOPER", "Devilmain3131 • Official release v1.0.0\nSupport the developer by following Devilmain3131!", "★", Color3.fromRGB(120, 90, 255), Color3.fromRGB(55, 145, 255), 72)

    local YTBtn = CreateButton(InfoPage, "YouTube • Synax Hub", function()
    ...
    local ReleaseVersion = CreateCard(InfoPage, "Version", Config.Version)
    ReleaseVersion.TextColor3 = Config.HighlightText
    ...
    LanguageInfo.Text = "15 languages available. Select one from the dropdown below."
    ...
    CreateToggle(SettingsPage, "Noclip (Walk Through Walls)", false, function(state)
        ...
        if state then
            UtilityState.NoclipConnection = trackConnection(RunService.Stepped:Connect(function()
                local char = LocalPlayer.Character
                if not char then return end
                for _, part in ipairs(char:GetDescendants()) do
                    if part:IsA("BasePart") then
    ...
    cleanupPlayerVisual = function(p)
        local data = ActiveDrawings[p]
        if data then
            for _, drawing in pairs(data) do
                if type(drawing) == "table" then
                    for _, item in pairs(drawing) do pcall(function() item.Visible = false; item:Remove() end) end
                elseif drawing then
    ...
    Players.PlayerAdded:Connect(function(player)
        player.CharacterAdded:Connect(function()
            if HubSettings.GlowAllWhite or HubSettings.BoxEsp or HubSettings.HealthBar or HubSettings.TargetLines or HubSettings.SkeletonEsp or HubSettings.NameDistanceEsp then
                task.wait()
                cleanupPlayerVisual(player)
            end
        end)
    end)
    for _, player in ipairs(Players:GetPlayers()) do
                        ...
                        if HubSettings.BoxEsp or HubSettings.SkeletonEsp then
                            if not ActiveDrawings[p] then
                                ...
                                pcall(function()
                                    ...
                                    if type(drawingNew) ~= "function" then return end
    ...
    local function isAimbotTargetValid(p)
        ...
        local targetPart = char:FindFirstChild(AimbotState.TargetPart) or char:FindFirstChild("Head") or char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or root
        ...
        if AimbotState.WallCheck then
            ...
            if hit and not hit.Instance:IsDescendantOf(char) then return false end
        end
    ...
    local function getBestAimbotTarget()
        if AimbotState.Target and isAimbotTargetValid(AimbotState.Target) then
    ...
    AimbotState.Connection = trackConnection(RunService.RenderStepped:Connect(function()
        ...
        local part = char and (char:FindFirstChild(AimbotState.TargetPart) or char:FindFirstChild("Head") or char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("HumanoidRootPart"))
        local camera = getAimbotCamera()
        ...
        local responsiveness = math.clamp(AimbotState.Smoothness, 0.01, 1)
        local alpha = 1 - math.pow(1 - responsiveness, 1)
        camera.CFrame = camera.CFrame:Lerp(desired, math.clamp(alpha, 0.01, 1))
    end))
    ...
    -----------------------------------------------------------------------
    -- [ BAN UI HELPER ]
    -- Client code cannot remove a server-side ban. The button closes the
    -- local ban overlay and requests a rejoin; the server remains authoritative.
    -----------------------------------------------------------------------
    local BanWatcherConnections = {}
    local function isBanGui(gui)
        local n = string.lower(gui.Name or "")
        return n:find("ban") or n:find("banned") or n:find("kick")
    end
    local function addRejoinButton(gui)
        if not gui or not gui:IsA("GuiObject") then return end
        if gui:FindFirstChild("SynaxUnbanRejoin") then return end
        local button = Instance.new("TextButton", gui)
        button.Name = "SynaxUnbanRejoin"
        button.Size = UDim2.new(0, 190, 0, 34)
        button.Position = UDim2.new(0.5, -95, 1, -48)
        button.BackgroundColor3 = Config.HighlightText
        button.BackgroundTransparency = 0.08
        button.BorderSizePixel = 0
        button.Text = "Unban & Rejoin"
    ...
    local function scanForBanGui(root)
        if not root then return end
        for _, obj in ipairs(root:GetDescendants()) do
            if (obj:IsA("Frame") or obj:IsA("TextLabel") or obj:IsA("TextButton")) and isBanGui(obj) then
                addRejoinButton(obj)
            end
            if obj:IsA("TextLabel") then
                local text = string.lower(obj.Text or "")
                if text:find("banned") or text:find("you are banned") or text:find("ban reason") then
                    local parent = obj.Parent
                    if parent and parent:IsA("GuiObject") then addRejoinButton(parent) end
                end
            end
        end
    end
    task.spawn(function()
        while MainGui.Parent do
            ...
            if pg then scanForBanGui(pg) end
            pcall(function() scanForBanGui(CoreGui) end)
        end
    end)
...
-- Protected entry point: a missing optional executor API must never abort the script.
local __ok, __err = pcall(ExecuteScript)
if not __ok then
    pcall(function()
        if CoreGui and CoreGui:FindFirstChild("SynaxHub") then
            CoreGui:FindFirstChild("SynaxHub"):Destroy()
        end
    end)
    warn("[SynaxHub] UI initialization failed safely: " .. tostring(__err))
end
