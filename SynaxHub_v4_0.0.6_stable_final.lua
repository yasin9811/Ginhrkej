...
-- Central connection registry: every RunService connection created by the
-- script can be disconnected during shutdown without leaving render loops alive.
local SynaxConnections = {}
local function trackConnection(connection)
    if connection then table.insert(SynaxConnections, connection) end
    return connection
end
local function disconnectSynaxConnections()
    for i = #SynaxConnections, 1, -1 do
        local connection = SynaxConnections[i]
        SynaxConnections[i] = nil
...
local function findToolEvent()
    ...
    -- Small, bounded fallback only inside the known Remotes/Tool area.
    if toolFolder then
        local direct = toolFolder:FindFirstChild("Event", true)
...
local function collectVendingMachines()
    ...
    -- Prefer explicitly named vending/machine folders. We never scan the whole
    -- Workspace every cycle; the result is cached by the caller.
    for _, name in ipairs({"VendingMachines", "Vending", "Machines", "Shop", "Shops"}) do
    ...
    if #candidates == 0 then
        for _, child in ipairs(Workspace:GetChildren()) do
            ...
            if n:find("vending") or n == "machines" or n:find("shop") then
    ...
    for _, folder in ipairs(candidates) do
        for _, inst in ipairs(folder:GetDescendants()) do
            if inst:IsA("RemoteEvent") and inst.Parent and inst.Parent:IsA("Configuration") then
                ...
                if lower:find("buy food", 1, true) or lower:find("buy drink", 1, true) then
                    local machine = inst:FindFirstAncestorOfClass("Model")
                    if machine then
                        table.insert(list, {
                            event = inst,
                            kind = lower:find("food", 1, true) and "Food" or "Drink",
                            model = machine,
...
-- Respawn-safe noclip collision cache. References from the old character are
-- discarded before the new character's parts are captured.
local AutoMineCharacterConnection = trackConnection(LocalPlayer.CharacterAdded:Connect(function()
    clearFarmTable(AutoMine.OriginalCollisions)
    task.defer(function()
        if AutoMine.IsActive then
            local char = LocalPlayer.Character
            if char then
...
local function mopPuddle(puddle)
    ...
    while JanitorState.AutoFarm do
        ...
        if (hrp.Position - puddle.Position).Magnitude > 8 then
            hrp.AssemblyLinearVelocity = Vector3.zero
            safeTeleportCFrame(hrp, CFrame.new(puddle.Position + Vector3.new(0, 3.2, 0)))
...
local function cleanNearestPuddle()
    ...
    safeTeleportCFrame(hrp, CFrame.new(nearest.Position + Vector3.new(0, 3.5, 0)))
    task.wait(JanitorState.TpWait)
...
local function safeTeleportCFrame(hrp, cframe)
    if not hrp or not hrp.Parent or not cframe then return false end
...
local function teleportToJanitor()
    ...
    if hrp then
        safeTeleportCFrame(hrp, CFrame.new(91.27, 13.8, -693.9))
    end
end
    ...
    task.spawn(function()
        for _, puddle in ipairs(getPuddles()) do
            if not JanitorState.AutoFarm then
                safeTeleportCFrame(hrp, CFrame.new(puddle.Position + Vector3.new(0, 3.5, 0)))
                task.wait(0.5)
            end
        end
    end)
            ...
            if targetRock then
                AutoMine.LockConnection = trackConnection(RunService.Heartbeat:Connect(function()
                    if targetRock and targetRock.Parent and rootPart and rootPart.Parent then
                        safeTeleportCFrame(rootPart, targetRock.CFrame * CFrame.new(0, 3, 0))
                    ...
                    if not trashTool then
                        ...
                        if TrashesFolder then
                            ...
                            if activeBin then
                                ...
                                if target then
                                    rootPart.AssemblyLinearVelocity = Vector3.zero
                                    safeTeleportCFrame(rootPart, target.CFrame * CFrame.new(0, 2, 0))
                        ...
                        if dumpster then
                            rootPart.AssemblyLinearVelocity = Vector3.zero
                            safeTeleportCFrame(rootPart, dumpster.CFrame * CFrame.new(0, 2, 0))
    ...
    while true do
        if FarmState.AutoCook then
            for index, step in ipairs(farmSteps) do
                ...
                if rootPart then
                    safeTeleportCFrame(rootPart, step.cframe)
                    task.wait(0.3)
...
local function ExecuteScript()
    ...
    -- Staff monitoring is dormant by default. It only scans after the user
    -- explicitly enables a staff feature.
    task.spawn(function()
        while MainGui.Parent do
            task.wait(StaffState.ScanInterval)
            if StaffState.Enabled then
                scanStaff()
            end
        end
    end)
    ...
    local function BuildStaffCategory(StaffPage, Config, CreateCard, CreateToggle, CreateButton)
        ...
        CreateToggle(StaffPage, "Staff ESP", false, function(state)
            ...
            StaffState.Enabled = StaffState.ESP or StaffState.Alerts or StaffState.FarmProtection or StaffState.KickOnStaffJoin
            if not state then
                for _, player in ipairs(Players:GetPlayers()) do
                    removeStaffESP(player)
                end
    ...
    CreateButton(FarmPage, "Teleport to Fishing Area", function()
        ...
        if hrp and fz then
            pcall(function()
                safeTeleportCFrame(hrp, fz:GetPivot() + Vector3.new(0, 5, 0))
            end)
        end
    end)
    ...
    local function teleportToBlackMarket()
        ...
        if npc and hrp then
            ...
            if npcHRP then
                ...
                hrp.AssemblyLinearVelocity = Vector3.zero
                safeTeleportCFrame(hrp, CFrame.lookAt(frontPosition.Position, npcHRP.Position))
            end
        end
    end
