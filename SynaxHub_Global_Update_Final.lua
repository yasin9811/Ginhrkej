...
-- Clean/local build: keep translations dependency-free.
-- Missing keys intentionally fall back to the key text itself.
local L = { English = setmetatable({}, {
    __index = function(_, key) return key end
}) }

L.Spanish = {
    ESP="ESP", Combat="Combate", Settings="Ajustes", ["Aimbot Toggle"]="Aimbot", ["FOV Slider"]="Deslizador FOV", ["Show FOV Circle"]="Mostrar círculo FOV",
    ["Legit Mode (Lerp)"]="Modo legítimo (Lerp)", ["Team Check"]="Comprobar equipo", ["Wall Check (Raycast)"]="Comprobar paredes (Raycast)", ["Velocity Predict"]="Predicción de velocidad",
    ["Glow ESP + Role Info"]="ESP de brillo + rol", ["Clean Health Bar (2D)"]="Barra de salud limpia (2D)", ["Status Tag ([WANTED]/[HOSTILE])"]="Etiqueta de estado ([WANTED]/[HOSTILE])",
    ["Inventory ESP (Icons)"]="ESP de inventario (iconos)", ["Distance Tracers"]="Trazadores de distancia", ["Zone Radius"]="Radio de zona", ["Special Player ESP (RGB)"]="ESP de jugador especial (RGB)",
    ["Refresh Player List"]="Actualizar lista de jugadores", ["Global Update"]="Actualización global", ["Optimization"]="Optimización", ["Language Support"]="Soporte de idiomas",
    ["ESP Engine"]="Motor ESP", ["Combat Engine"]="Motor de combate", ON="ACTIVADO", OFF="DESACTIVADO"
}
L.German = {
    ESP="ESP", Combat="Kampf", Settings="Einstellungen", ["Aimbot Toggle"]="Aimbot", ["FOV Slider"]="FOV-Regler", ["Show FOV Circle"]="FOV-Kreis anzeigen",
    ["Legit Mode (Lerp)"]="Legit-Modus (Lerp)", ["Team Check"]="Teamprüfung", ["Wall Check (Raycast)"]="Wandprüfung (Raycast)", ["Velocity Predict"]="Geschwindigkeitsvorhersage",
    ["Glow ESP + Role Info"]="Glow ESP + Rolleninfo", ["Clean Health Bar (2D)"]="Saubere Gesundheitsleiste (2D)", ["Status Tag ([WANTED]/[HOSTILE])"]="Status-Tag ([WANTED]/[HOSTILE])",
    ["Inventory ESP (Icons)"]="Inventar-ESP (Symbole)", ["Distance Tracers"]="Distanzlinien", ["Zone Radius"]="Zonenradius", ["Special Player ESP (RGB)"]="Spezialspieler-ESP (RGB)",
    ["Refresh Player List"]="Spielerliste aktualisieren", ["Global Update"]="Globales Update", ["Optimization"]="Optimierung", ["Language Support"]="Sprachunterstützung",
    ["ESP Engine"]="ESP-Engine", ["Combat Engine"]="Kampf-Engine", ON="AN", OFF="AUS"
}
L.French = {
    ESP="ESP", Combat="Combat", Settings="Paramètres", ["Aimbot Toggle"]="Aimbot", ["FOV Slider"]="Curseur FOV", ["Show FOV Circle"]="Afficher le cercle FOV",
    ["Legit Mode (Lerp)"]="Mode légitime (Lerp)", ["Team Check"]="Vérification d'équipe", ["Wall Check (Raycast)"]="Vérification des murs (Raycast)", ["Velocity Predict"]="Prédiction de vitesse",
    ["Glow ESP + Role Info"]="ESP Glow + rôle", ["Clean Health Bar (2D)"]="Barre de santé propre (2D)", ["Status Tag ([WANTED]/[HOSTILE])"]="Tag de statut ([WANTED]/[HOSTILE])",
    ["Inventory ESP (Icons)"]="ESP inventaire (icônes)", ["Distance Tracers"]="Traceurs de distance", ["Zone Radius"]="Rayon de zone", ["Special Player ESP (RGB)"]="ESP joueur spécial (RGB)",
    ["Refresh Player List"]="Actualiser la liste", ["Global Update"]="Mise à jour globale", ["Optimization"]="Optimisation", ["Language Support"]="Prise en charge des langues",
    ["ESP Engine"]="Moteur ESP", ["Combat Engine"]="Moteur Combat", ON="ACTIVÉ", OFF="DÉSACTIVÉ"
}
L.Italian = {
    ESP="ESP", Combat="Combattimento", Settings="Impostazioni", ["Aimbot Toggle"]="Aimbot", ["FOV Slider"]="Cursore FOV", ["Show FOV Circle"]="Mostra cerchio FOV",
    ["Legit Mode (Lerp)"]="Modalità legittima (Lerp)", ["Team Check"]="Controllo squadra", ["Wall Check (Raycast)"]="Controllo muri (Raycast)", ["Velocity Predict"]="Predizione velocità",
    ["Glow ESP + Role Info"]="Glow ESP + ruolo", ["Clean Health Bar (2D)"]="Barra salute pulita (2D)", ["Status Tag ([WANTED]/[HOSTILE])"]="Tag stato ([WANTED]/[HOSTILE])",
    ["Inventory ESP (Icons)"]="ESP inventario (icone)", ["Distance Tracers"]="Tracciatori distanza", ["Zone Radius"]="Raggio zona", ["Special Player ESP (RGB)"]="ESP giocatore speciale (RGB)",
    ["Refresh Player List"]="Aggiorna lista giocatori", ["Global Update"]="Aggiornamento globale", ["Optimization"]="Ottimizzazione", ["Language Support"]="Supporto lingue",
    ["ESP Engine"]="Motore ESP", ["Combat Engine"]="Motore combattimento", ON="ATTIVO", OFF="DISATTIVO"
}
L.English = setmetatable({
    ESP="ESP", Combat="Combat", Settings="Settings", ["Aimbot Toggle"]="Aimbot Toggle", ["FOV Slider"]="FOV Slider", ["Show FOV Circle"]="Show FOV Circle",
    ["Legit Mode (Lerp)"]="Legit Mode (Lerp)", ["Team Check"]="Team Check", ["Wall Check (Raycast)"]="Wall Check (Raycast)", ["Velocity Predict"]="Velocity Predict",
    ["Glow ESP + Role Info"]="Glow ESP + Role Info", ["Clean Health Bar (2D)"]="Clean Health Bar (2D)", ["Status Tag ([WANTED]/[HOSTILE])"]="Status Tag ([WANTED]/[HOSTILE])",
    ["Inventory ESP (Icons)"]="Inventory ESP (Icons)", ["Distance Tracers"]="Distance Tracers", ["Zone Radius"]="Zone Radius", ["Special Player ESP (RGB)"]="Special Player ESP (RGB)",
    ["Refresh Player List"]="Refresh Player List", ["Global Update"]="Global Update", ["Optimization"]="Optimization", ["Language Support"]="Language Support",
    ["ESP Engine"]="ESP Engine", ["Combat Engine"]="Combat Engine", ON="ON", OFF="OFF"
}, {__index=function(_, key) return key end})
...
-- Server-side authorization through the C11 Synax API.
-- No Discord webhook/bot is used by this live-control system.
-- ============================================================================

-- Current public API endpoint.
-- NOTE: Cloudflare Quick Tunnel URLs can change when the tunnel is restarted.
local C11_ACCESS_API_URL = "https://internationally-possibly-constitution-coins.trycloudflare.com"

-- Request adapter must exist BEFORE the first access check.
...
local function checkC11Access()
    ...
    local endpoint = C11_ACCESS_API_URL ..
        "/api/access/" .. tostring(LocalPlayer.UserId)
...
local function castRod()
    local ok = pcall(function()
        if not loadFishingModules() or not PowerBarSystem then return end
        ...
        while os.clock() - startTime < 5 and FarmState.AutoFish do
            task.wait(0.05)
            local charging = false
            pcall(function() charging = PowerBarSystem:IsCharging() end)
            local power = 0
            pcall(function() power = PowerBarSystem:GetCurrentPower() end)
            if power >= 99.5 then break end
            if not charging and os.clock() - startTime > 0.5 then break end
        end
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        if not loadFishingModules() or not MinigameSystem then return end
        if not MinigameSystem:IsActive() then return end

        local phase = MinigameSystem:GetPhase()
        if phase == "shake" then
            if os.clock() - lastClick > 0.06 then
                pcall(function() MinigameSystem:HandleClick(SoundManager, GUIManager) end)
                lastClick = os.clock()
            end
        elseif phase == "reel" then
            ...
            if zone and marker then
                ...
                pcall(function() MinigameSystem:SetHolding(fishPos > zonePos + 0.01) end)
            end
        end
    end)
end)
...
local function pollLiveSystems(MainGui)
    ...
    pcall(function()
        local statusResponse = req({
            Url = C11_ACCESS_API_URL .. "/api/status",
            Method = "GET",
            Headers = { ["Content-Type"] = "application/json" }
        })
        local statusData = getJsonBody(statusResponse)
        if statusData and statusData.scriptEnabled == false then
            scriptDisabled = true
            C11AccessState.Allowed = false
            C11AccessState.Reason = statusData.maintenanceMessage or "Script is currently disabled."
            C11AccessState.ExpiresAt = nil
            C11AccessState.CheckedAt = os.time()
            C11AccessState.APIOnline = true
            if MainGui and MainGui.Parent then MainGui:Destroy() end
            showRestrictionUI(C11AccessState.Reason, nil, true)
        end
    end)
    if scriptDisabled then return false end

    local ok, response = pcall(function()
        return req({
            Url = C11_ACCESS_API_URL .. "/api/access/" .. tostring(LocalPlayer.UserId),
            Method = "GET",
            Headers = {
                ["Content-Type"] = "application/json"
            }
    ...
    if announcementOk then
        ...
        local announcement = data and (data.announcement or data)
        if announcement and announcement.active == true and announcement.id then
            local numericId = tonumber(announcement.id)
            if numericId and (lastAnnouncementId == nil or numericId > lastAnnouncementId) then
                lastAnnouncementId = numericId
                showLiveAnnouncement(announcement.message, announcement.createdBy)
            end
        end
    end
    ...
    task.spawn(function()
        ...
        if req then
            pcall(function()
                ...
                local data = getJsonBody(response)
                local announcement = data and (data.announcement or data)
                if announcement and announcement.active and announcement.id then
                    lastAnnouncementId = tonumber(announcement.id)
                end
            end)
        end
...
local function ExecuteScript()
    ...
    -----------------------------------------------------------------------
    -- [ ESP TAB • GLOBAL UPDATE ENGINE ]
    -----------------------------------------------------------------------
    CreateToggle(EspPage, "Glow ESP + Role Info", false, function(state)
        ESPState.Glow = state
    end)

    CreateToggle(EspPage, "Clean Health Bar (2D)", false, function(state)
        ESPState.Health = state
    end)

    CreateToggle(EspPage, "Status Tag ([WANTED]/[HOSTILE])", false, function(state)
        ESPState.Status = state
    end)

    CreateToggle(EspPage, "Inventory ESP (Icons)", false, function(state)
        ESPState.Inventory = state
    end)

    CreateToggle(EspPage, "Distance Tracers", false, function(state)
        ESPState.Tracers = state
    end)

    CreateFarmSlider(EspPage, "Zone Radius", 20, 500, 150, 10, " studs", function(value)
        ESPState.ZoneRadius = value
        ESPState.ZonePulseUntil = os.clock() + 1.5
    end)

    local ESPPlayerSelector = CreateCard(EspPage, "Special Player", "None")
    CreateButton(EspPage, "Refresh Player List", function()
        ESPState.SpecialTarget = nil
        ESPPlayerSelector.Text = "None"
    end)

    CreateToggle(EspPage, "Special Player ESP (RGB)", false, function(state)
        ESPState.Special = state
    end)

    local ESPInfo = Instance.new("TextLabel", EspPage)
    ESPInfo.Size = UDim2.new(1, -10, 0, 52)
    ESPInfo.BackgroundTransparency = 1
    ESPInfo.Text = "Global ESP Engine • Glow + role info, 2D health, status tags, inventory icons, distance tracers, zone radius and RGB special-player highlighting."
    ESPInfo.TextColor3 = Config.TextSecondary
    ESPInfo.Font = Enum.Font.Gotham
    ESPInfo.TextSize = 10
    ESPInfo.TextWrapped = true
    ESPInfo.TextXAlignment = Enum.TextXAlignment.Left

    -- Compact player selector: tap the card to cycle through active players.
    local selectorFrame = ESPPlayerSelector.Parent
    if selectorFrame then
        ...
        selectorFrame.InputBegan:Connect(function(input)
            ...
            ESPState.SpecialIndex = (ESPState.SpecialIndex % #list) + 1
            local target = list[ESPState.SpecialIndex]
            ESPState.SpecialTarget = target
            ESPPlayerSelector.Text = target.Name
        end)
    end
    ...
    -----------------------------------------------------------------------
    -- [ COMBAT TAB • GLOBAL UPDATE ENGINE ]
    -----------------------------------------------------------------------
    CreateToggle(CombatPage, "Aimbot Toggle", false, function(state)
        AimbotState.MobileEnabled = state
        AimbotState.Enabled = state
        if not state then AimbotState.Target = nil end
    end)

    CreateFarmSlider(CombatPage, "FOV Slider", 10, 300, 80, 1, "", function(value)
        AimbotState.FOV = value
    end)

    CreateToggle(CombatPage, "Show FOV Circle", true, function(state)
        AimbotState.FOVCircleVisible = state
    end)

    CreateToggle(CombatPage, "Legit Mode (Lerp)", true, function(state)
        AimbotState.LegitMode = state
    end)
    ...
    CreateToggle(CombatPage, "Wall Check (Raycast)", false, function(state)
        AimbotState.WallCheck = state
    end)

    CreateToggle(CombatPage, "Velocity Predict", false, function(state)
        AimbotState.PredictionEnabled = state
    end)

    local CombatInfo = Instance.new("TextLabel", CombatPage)
    CombatInfo.Size = UDim2.new(1, -10, 0, 42)
    CombatInfo.BackgroundTransparency = 1
    CombatInfo.Text = "Combat Engine • FOV targeting, optional team/raycast checks, smooth Lerp aiming and velocity prediction."
    CombatInfo.TextColor3 = Config.TextSecondary
    ...
    UpdateInfoTitle.Text = "SynaxHub Global Update • beta - 0.0.6"
    ...
    UpdateInfoDesc.Size = UDim2.new(1, -10, 0, 82)
    UpdateInfoDesc.BackgroundTransparency = 1
    UpdateInfoDesc.Text = "GLOBAL UPDATE\nNew optimization routines reduce unnecessary visual work and improve cleanup stability. Global language support now includes EN, ES, DE, FR and IT. The ESP engine adds glow and role information, clean 2D health bars, status tags, inventory icons, distance tracers, zone radius and RGB special-player ESP. The Combat engine adds FOV targeting, Show Circle, Legit Mode with Lerp, Team Check, Wall Check with Raycast and optional Velocity Prediction."
    ...
    UpdateInfoDesc.TextYAlignment = Enum.TextYAlignment.Top

    local OptimizationTitle = Instance.new("TextLabel", InfoPage)
    OptimizationTitle.Size = UDim2.new(1, -10, 0, 18)
    OptimizationTitle.BackgroundTransparency = 1
    OptimizationTitle.Text = "Global Optimization & Language Support"
    OptimizationTitle.TextColor3 = Config.HighlightText
    OptimizationTitle.Font = Enum.Font.GothamBold
    OptimizationTitle.TextSize = 12
    OptimizationTitle.TextXAlignment = Enum.TextXAlignment.Left

    local OptimizationDesc = Instance.new("TextLabel", InfoPage)
    OptimizationDesc.Size = UDim2.new(1, -10, 0, 45)
    OptimizationDesc.BackgroundTransparency = 1
    OptimizationDesc.Text = "Five-second memory cleanup, player-removal cleanup and executor-friendly task.wait/pcall protections are integrated across the updated runtime."
    OptimizationDesc.TextColor3 = Config.TextSecondary
    OptimizationDesc.Font = Enum.Font.Gotham
    OptimizationDesc.TextSize = 10
    OptimizationDesc.TextWrapped = true
    OptimizationDesc.TextXAlignment = Enum.TextXAlignment.Left
    ...
    -----------------------------------------------------------------------
    -- [ GLOBAL ESP ENGINE ]
    -- Integrated directly into SynaxHub; no separate ESP UI is used.
    -----------------------------------------------------------------------
        local ESPDrawings = {}
    local ESPZoneCircle = nil
    pcall(function()
        ESPZoneCircle = Drawing.new("Circle")
        ESPZoneCircle.Visible = false
        ESPZoneCircle.Thickness = 1.5
        ESPZoneCircle.NumSides = 48
        ESPZoneCircle.Filled = false
    end)

    local function espRemoveDrawing(obj)
        if obj then pcall(function() obj.Visible = false; obj:Remove() end) end
    end

    local function espCleanupPlayer(player)
        local key = player and player.UserId
        if key and ESPDrawings[key] then
            local d = ESPDrawings[key]
            for _, obj in pairs(d) do
                if type(obj) == "table" then
                    for _, line in ipairs(obj) do espRemoveDrawing(line) end
                else
                    espRemoveDrawing(obj)
                end
            end
            ESPDrawings[key] = nil
        end
        if player and player.Character then
            for _, name in ipairs({"SynaxGlobalGlow", "SynaxGlobalESPTag", "SynaxGlobalInventory"}) do
    ...
    local function espGetRole(player)
        if player.Team then
    ...
    local function espGetStatus(player)
        local tags = {}
        local wanted = player:FindFirstChild("Wanted")
        local leaderstats = player:FindFirstChild("leaderstats")
        wanted = wanted or (leaderstats and (leaderstats:FindFirstChild("Wanted") or leaderstats:FindFirstChild("Wanted Level")))
        if wanted then
            local value = wanted.Value
            if value == true or (type(value) == "number" and value > 0) then table.insert(tags, "[WANTED]") end
        end
        local team = player.Team
        if team then
            local n = string.lower(team.Name)
            if (n:find("wanted") or n:find("aranıyor")) and not table.find(tags, "[WANTED]") then table.insert(tags, "[WANTED]") end
            if n:find("hostile") or n:find("criminal") or n:find("raider") or n:find("düşman") then table.insert(tags, "[HOSTILE]") end
        end
        return table.concat(tags, " ")
    end

    local function espGetTools(player)
        local result = {}
        local backpack = player:FindFirstChildOfClass("Backpack")
        local char = player.Character
        if backpack then
            for _, tool in ipairs(backpack:GetChildren()) do if tool:IsA("Tool") then table.insert(result, tool) end end
        end
        if char then
            for _, tool in ipairs(char:GetChildren()) do if tool:IsA("Tool") then table.insert(result, tool) end end
        end
        return result
    end

    local function espEnsureDrawings(player)
        local key = player.UserId
        if ESPDrawings[key] then return ESPDrawings[key] end
        local d = {}
        pcall(function()
    ...
    local function espSetBillboard(player, name, text, offset, color, size)
        local char = player.Character
        ...
        if not gui then
            ...
            gui.Name = name
            gui.Size = size or UDim2.fromOffset(190, 45)
            gui.StudsOffset = offset or Vector3.new(0, 3.2, 0)
    ...
    local function espInventory(player, screenPos)
        local char = player.Character
        local head = char and char:FindFirstChild("Head")
        if not head then return end
        local gui = head:FindFirstChild("SynaxGlobalInventory")
        if not gui then
            gui = Instance.new("BillboardGui")
            gui.Name = "SynaxGlobalInventory"
        ...
        local tools = espGetTools(player)
        for i, tool in ipairs(tools) do
            if i > 5 then break end
            local image = Instance.new("ImageLabel")
            image.BackgroundTransparency = 1
            image.Size = UDim2.fromOffset(22, 22)
            image.Position = UDim2.fromOffset((i - 1) * 25, 2)
            image.Image = (tool.TextureId and tool.TextureId ~= "") and tool.TextureId or "rbxassetid://6071575925"
            image.Parent = gui
        end
    end

    local function espClearPlayerState(player)
        if player then espCleanupPlayer(player) end
    end

    Players.PlayerRemoving:Connect(espClearPlayerState)

    -- Required five-second memory cleanup.
    task.spawn(function()
        while MainGui.Parent do
            task.wait(5)
            pcall(function()
                for _, player in ipairs(Players:GetPlayers()) do
                    if player ~= LocalPlayer and not player.Character then espCleanupPlayer(player) end
                end
    ...
    task.spawn(function()
        while MainGui.Parent do
            ...
            local rainbow = Color3.fromHSV((os.clock() * 0.6) % 1, 0.9, 1)

            if ESPZoneCircle then
                ESPZoneCircle.Position = center
                ESPZoneCircle.Radius = ESPState.ZoneRadius
                ESPZoneCircle.Color = rainbow
                ESPZoneCircle.Visible = os.clock() < ESPState.ZonePulseUntil
            end
            ...
            for _, player in ipairs(Players:GetPlayers()) do
                ...
                if not root or not head then
                    espCleanupPlayer(player)
                    continue
                end

                local roleName, roleColor = espGetRole(player)
                ...
                local d = espEnsureDrawings(player)

                local glow = char:FindFirstChild("SynaxGlobalGlow")
                if ESPState.Glow then
                    if not glow then
                        glow = Instance.new("Highlight")
                        glow.Name = "SynaxGlobalGlow"
                        glow.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                        glow.Parent = char
                    end
                    glow.FillTransparency = 1
                    glow.OutlineTransparency = 0
                    glow.OutlineColor = roleColor
                    local status = ESPState.Status and espGetStatus(player) or ""
                    local extra = status ~= "" and "\n" .. status or ""
                    espSetBillboard(player, "SynaxGlobalESPTag", string.format("@%s [%dm]\n[%s]%s", player.Name, distance, roleName, extra), Vector3.new(0, 3.2, 0), roleColor, UDim2.fromOffset(190, 48))
                elseif glow then
                    glow:Destroy()
                end

                local tag = char:FindFirstChild("SynaxGlobalESPTag")
                if not ESPState.Glow and tag then tag:Destroy() end

                if d.HealthBg and ESPState.Health and hum and headVisible then
                    local h = math.clamp(1000 / math.max(distance, 1), 20, 50)
                    local hp = math.clamp(hum.Health / math.max(hum.MaxHealth, 1), 0, 1)
                    d.HealthBg.Size = Vector2.new(4, h)
                    d.HealthBg.Position = Vector2.new(headPos.X - 20, headPos.Y - h / 2)
                    d.HealthBg.Visible = true
                    d.HealthFill.Size = Vector2.new(4, h * hp)
                    d.HealthFill.Position = Vector2.new(headPos.X - 20, headPos.Y - h / 2 + h * (1 - hp))
                ...
                if ESPState.Inventory and headVisible then
                    espInventory(player, headPos)
                else
                    local inv = char:FindFirstChild("SynaxGlobalInventory")
                    if inv then inv:Destroy() end
                end

                if d.Tracer and ESPState.Tracers and localRoot and rootVisible and headVisible then
                    local from = camera:WorldToViewportPoint(localRoot.Position)
                    local dist = (localRoot.Position - root.Position).Magnitude
                    d.Tracer.From = Vector2.new(from.X, from.Y)
                    d.Tracer.To = Vector2.new(rootPos.X, rootPos.Y)
                    d.Tracer.Color = dist <= 30 and Color3.fromRGB(0, 255, 100) or dist <= 80 and Color3.fromRGB(255, 170, 0) or Color3.fromRGB(255, 50, 50)
                    d.Tracer.Visible = dist <= ESPState.ZoneRadius
                elseif d.Tracer then
                    d.Tracer.Visible = false
                end
            end

            if ESPState.Special and ESPState.SpecialTarget and ESPState.SpecialTarget.Parent then
                local p = ESPState.SpecialTarget
                local char = p.Character
                if char then
                    local glow = char:FindFirstChild("SynaxGlobalSpecialRGB")
                    if not glow then
                        glow = Instance.new("Highlight")
                        glow.Name = "SynaxGlobalSpecialRGB"
                        glow.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
                        glow.FillTransparency = 1
                        glow.OutlineTransparency = 0
                        glow.Parent = char
                    end
                    glow.OutlineColor = rainbow
                end
            else
                for _, p in ipairs(Players:GetPlayers()) do
                    if p.Character then
                        local glow = p.Character:FindFirstChild("SynaxGlobalSpecialRGB")
    ...
    AimbotState.Connection = RunService.RenderStepped:Connect(function()
        local camera = getAimbotCamera()
        if AimbotFOVCircle and camera then
            AimbotFOVCircle.Position = Vector2.new(camera.ViewportSize.X / 2, camera.ViewportSize.Y / 2)
            AimbotFOVCircle.Radius = AimbotState.FOV
            AimbotFOVCircle.Visible = AimbotState.FOVCircleVisible
        end
        ...
        local predicted = AimbotState.PredictionEnabled and (part.Position + velocity * AimbotState.Prediction) or part.Position
        local desired = CFrame.lookAt(camera.CFrame.Position, predicted)
        if AimbotState.LegitMode then
