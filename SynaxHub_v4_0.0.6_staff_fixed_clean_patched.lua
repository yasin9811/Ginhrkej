...
-- Runtime-safe ping reader.
-- Some executors expose Stats differently; never call a missing GetValue method.
local function getSafePing()
    local ok, ping = pcall(function()
        ...
        local dataPing = items and items:FindFirstChild("Data Ping")

        if not dataPing then
            return nil
        end

        local getter = dataPing.GetValue
        if type(getter) ~= "function" then
            return nil
        end

        return getter(dataPing)
    end)
...
local function ExecuteScript()
    ...
    -----------------------------------------------------------------------
    local StaffState = {
        -- Monitoring is always active; the toggles below control optional actions.
        Enabled = true,
        ESP = false,
        Alerts = false,
    ...
    local function getStaffData(player)
        ...
        local normalizedRole = string.lower(string.gsub(tostring(role), "^%s*(.-)%s*$", "%1"))
        for _, data in pairs(STAFF_ROLES) do
            if normalizedRole == string.lower(data.name) then
                return data
            end
        end
        ...
        if rankOk and tonumber(rank) then
            for _, data in pairs(STAFF_ROLES) do
                if tonumber(data.rank) == tonumber(rank) then
                    return data
                end
            end
        end
    ...
    task.spawn(function()
        while MainGui.Parent and task.wait(1.5) do
            ...
            PingLabel.Text = ping and (tostring(ping) .. " ms") or "Unavailable"
    ...
    WebhookInfo.Text = "Execution logs include username, User ID, game, Place ID, server Job ID and time."
...
local _startupOk, _startupErr = pcall(ExecuteScript)
if not _startupOk then
    warn("[Synax Hub] Startup error: " .. tostring(_startupErr))
end
