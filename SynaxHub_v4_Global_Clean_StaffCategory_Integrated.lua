...
-- ============================================================================
-- STAFF CATEGORY INTEGRATION
-- Integrated into the existing Global Clean Staff page.
-- No separate SynaxStaffUI / ScreenGui is created.
-- ============================================================================

local STAFF_GROUP_ID = 304256484
local STAFF_MIN_RANK = 120

local StaffMonitorState = {
    ESP = false,
    Alerts = false,
    FarmProtection = false,
    LeaveOnJoin = false,
...
local function IsConfiguredStaff(player)
    ...
    return rank >= STAFF_MIN_RANK, rank
end

local function GetStaffSnapshot()
    local result = {}

    for _, player in ipairs(Players:GetPlayers()) do
        local staff, rank = IsConfiguredStaff(player)
        if staff then
            table.insert(result, {
                Player = player,
                Rank = rank,
...
-- The existing Staff page should consume these values instead of creating a
-- second GUI.  Existing UI helper names are detected conservatively.
local function BindStaffCategory()
    local page = rawget(_G, "StaffPage")
    if not page then
        return
    end

    -- Expose the state/config to the existing page.
    page:SetAttribute("StaffGroupId", STAFF_GROUP_ID)
    page:SetAttribute("StaffMinimumRank", STAFF_MIN_RANK)

    -- If the Global Clean build provides its own toggle/button helpers, attach
    -- the controls to the existing page. No new ScreenGui is created here.
    local toggle = rawget(_G, "CreateToggle")
    local button = rawget(_G, "CreateButton")

    if type(toggle) == "function" then
        toggle(page, "Staff ESP", false, function(value)
            StaffMonitorState.ESP = value
        end)

        toggle(page, "Staff Alerts", false, function(value)
            StaffMonitorState.Alerts = value
        end)

        toggle(page, "Farm Protection", false, function(value)
            StaffMonitorState.FarmProtection = value
        end)

        toggle(page, "Leave on Staff Join", false, function(value)
            StaffMonitorState.LeaveOnJoin = value
        end)
    end

    if type(button) == "function" then
        button(page, "SCAN STAFF", function()
            local staff = GetStaffSnapshot()
            warn(string.format("[SynaxHub] Staff scan: %d staff member(s) detected.", #staff))
            for _, entry in ipairs(staff) do
                warn(string.format("[SynaxHub] %s | rank %d", entry.Player.Name, entry.Rank))
            end
        end)
    end

    _G.SynaxStaffMonitor = {
        GroupId = STAFF_GROUP_ID,
        MinimumRank = STAFF_MIN_RANK,
        Config = StaffMonitorState,
        IsStaff = IsConfiguredStaff,
        Scan = GetStaffSnapshot,
    }
end

task.defer(BindStaffCategory)
