...
-- ============================================================================
-- SYNAX EXECUTION LOGGER (Discord Webhook)
-- Paste your own webhook URL into WEBHOOK_URL locally.
...
local function sendExecutionLog()
    if not EXECUTION_LOGGING then return end
    ...
    local payload = {
        ...
        embeds = {{
            ...
            fields = {
                ...
                {name = "Job ID", value = tostring(game.JobId ~= "" and game.JobId or "Studio/Private"), inline = false},
...
-- ============================================================================
-- AUTO TRASH - EXACT SEQUENCE
-- TP -> take trash -> TP dumpster -> wait 2s -> throw -> repeat
-- ============================================================================
        ...
        while AutoTrash.IsActive do
            ...
            if char and backpack then
                pcall(function()
                    ...
                    -- 1) Çöp elde değilse: aktif çöp noktasına TP -> çöpü al.
                    local trashTool =
                    ...
                    if not trashTool then
                        ...
                        local prisoner = tasks and tasks:FindFirstChild("Prisoner")
                        local trashes = prisoner and prisoner:FindFirstChild("Trashes")
                        local activeBin

                        if trashes then
                            for _, bin in ipairs(trashes:GetChildren()) do
                        ...
                        -- 2) Çöp alındı: çöpe TP -> TAM 2 saniye -> at.
                        ...
                        if dumpster then
                            ...
                            if trashTool.Parent == backpack then
                                pcall(function() humanoid:EquipTool(trashTool) end)
                                task.wait(0.15)
                            end
